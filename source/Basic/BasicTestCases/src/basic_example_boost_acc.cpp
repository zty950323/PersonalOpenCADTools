...
namespace Basic 
{
    int TzBasicBoostAccExample::runAllTestCase(int argc, char* argv[])
    {
        exampleForSetByForEach();
        exampleForStatics();
        exampleForWeightedStatistics();
        return 1;
    }

    void TzBasicBoostAccExample::exampleForSetByForEach()
    {
        accumulator_set<
            double,
        ...
        boost::array<double, 4> data = { 0.0, 1.0, -1.0, 3.14159 };

        // std::for_each pushes each sample into the accumulator one at a time,
        // and returns a copy of the accumulator.
        // acc = std::for_each(data.begin(), data.end(), acc);

        // The following would be equivalent, and could be more efficient because 
        // it doesn't pass and return the entire accumulator set by value.
        std::for_each(data.begin(), data.end(), bind<void>(ref(acc), _1));

        std::cout << "  min""(acc)      = " << (min)(acc) << std::endl;
        // Extra quotes are to prevent complaints from Boost inspect tool
    ...
    void TzBasicBoostAccExample::exampleForStatics()
    {
    ...
    void TzBasicBoostAccExample::exampleForWeightedStatistics()
    {
        // weight == double
        double w = 1.;
