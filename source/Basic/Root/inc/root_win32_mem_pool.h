...
#ifndef _ROOT_WIN32_MEM_POOL_H_H_
#define _ROOT_WIN32_MEM_POOL_H_H_

#include "zsofts_compiler_specific.h"

#if (TZ_PLATFORM == TZ_PLATFORM_WINDOWS) && (TZ_ARCHITECURE_BIT == TZ_ARCHITECTURE_BIT32)
...
private:
	...
	static const short s_unitSizeOfPools[kPoolCount + 1];
};

class TzHeapConfigurator
{
public:
	// static short kMaxPageId;
	static short mOffSetId;
	...
	enum
	{
		...
		kMaxPages = 0x0c00, /// *0x0800*/0x2000
		kMaxPageId = kMaxPages - 1,
		kFirstFreePageId = 0x07EF,
		kBlockBits = kSectionBits - kFactorBits,	// kSectionBits - kFactorBits
		...
		kBlksPerSectionBits = kSectionBits - kBlockBits,
	...
	static short								sGetPageId(const void* p);
	static bool								sIsOutOfHeapRange(const void* p);
	static TZADDRESS					sGetPageAddress(short pageId);
	static TZADDRESS					sGetPageAddress(const void* p);
	static short								sGetBlockId(const void* p);
	static TZADDRESS					sGetBlockOffset(const void* p);
	static TZADDRESS					sGetBlockAddress(const void* p);

	static void*								sMemoryAlloc(void *p, size_t size, TZUINT32 allocType, TZUINT32 protect);
...
TZ_FORCEINLINE
short TzHeapConfigurator::sGetPageId(const void* p)
{
	return (short)(((TZADDRESS)p) >> kPageBits);
}
...
bool TzHeapConfigurator::sIsOutOfHeapRange(const void* p)
{
	...
	return
		lPageid > kMaxPageId
#ifndef TZ_WINDOWS
		|| lPageid < TzHeapConfigurator::sGetPageId(TzHeapConfigurator::pStartAddress)
#endif
		;
}

TZ_FORCEINLINE
TZADDRESS TzHeapConfigurator::sGetPageAddress(short pageId)
{
	return ((TZADDRESS)pageId) << kPageBits;
}
...
TZ_FORCEINLINE
short TzHeapConfigurator::sGetBlockId(const void* p)
{
	return (short)((((TZADDRESS)p) >> kBlockBits) & kMaxBlksPerPage);
}
...
TZ_FORCEINLINE
void * TzHeapConfigurator::sMemoryAlloc(void* p, size_t size, TZUINT32 allocType, TZUINT32 protect)
{
	void * pNewMem = NULL;
...
int TzHeapConfigurator::sGetPagePoolIndex(size_t size)
{
	...
	return (int)(size >> kBlockBits);
}

size_t TzHeapConfigurator::sGetSizeOfPagePool(int poolIndex)
{
	return (size_t)(poolIndex << kBlockBits);
}
...
class TzHeap
{
	...
	public:
		...
		short parentPageId() const
		{
			return HeapCfg::sGetPageId(this);
		}

		Page * parentPage()
		{
			return (Page*)HeapCfg::sGetPageAddress(this);
		}
	...
	{
	public:
		Page* prevPage() const { return (Page*)HeapCfg::sGetPageAddress((short)prevPageId); }
		Page* nextPage() const { return (Page*)HeapCfg::sGetPageAddress((short)nextPageId); }
	...
	public:
			...
			m_pFullBlock(NULL)
		{
			...
			if (m_nUnitSize > 0)
			{
				m_nMaxUnitCount = (int)(HeapCfg::kBlockSize / m_nUnitSize);
			}
		}
	...
	public:
		...
		Unit* firstUnit() const
		{
			return (Unit*)((TZADDRESS)pageId << HeapCfg::kPageBits);
		}

		Unit* lastUnit(size_t size) const
		{
			return (Unit*)(((TZADDRESS)(pageId + 1) << HeapCfg::kPageBits) - size);
		}
	...
	public:
			...
			m_pFullPageHeap(NULL)
		{
			...
			if (m_nUnitSize > 0)
			{
				m_nMaxUnitCount = (int)(HeapCfg::kPageSize / m_nUnitSize);
			}
		}
	...
	public:
		...
		int							m_nPoolIndex;
		int							m_nMaxUnitCount;
		PageHeap*				m_pFreePageHeap;
		PageHeap*				m_pFullPageHeap;
	};

public:
	bool isPageAllocated(short pageId)
	{
		return TZGETBIT(m_pageFlags[pageId - HeapCfg::mOffSetId], HeapCfg::kAllocated);
	}

	void setPageAllocated(short pageId, bool bAlloc = true)
	{
		TZGETBIT(m_pageFlags[pageId - HeapCfg::mOffSetId], HeapCfg::kAllocated, bAlloc);
	}
	...
	void* preallocPageHeap(short fromPageId, short toPageId, TZBYTE pageFlag)
	{
...
#ifdef TZ_WINDOWS
				pNewPage = VirtualAlloc(pAddress, HeapCfg::kPageSize, MEM_RESERVE, PAGE_READWRITE);
#else
				pNewPage = pAddress;
#endif
				if (pNewPage)
	...
	void freeRawPage(void* pPage)
	{
		...
		short pageId = HeapCfg::sGetPageId(pPage);
	...
	Page* allocPage()
	{
		Page* pNewPage = (Page*)preallocNewPage();
		TZ_ASSERT(pNewPage);
		if (!pNewPage)
		{
			return NULL;
	...
	void reallocBlockFromPage(Page* pPage)
	{
		...
		if (pPage->leftBlock)
		{
			pAddress = (void*)pPage->leftBlock->firstUnit();
		}
	...
	void freeBlock(Block* pBlock)
	{
		...
		{
			if ((pPage->currBlockCount + pPage->leftBlockCount != (HeapCfg::kBlksPerPageMask - 1)) ||
	...
	void* reallocFromPool(Pool* pPool)
	{
		...
		if (pBlock == NULL)
		{
			...
			pBlock->freeCount = (short)pPool->m_nMaxUnitCount;
		}
	...
	void* allocFromPool(Pool* pPool)
	{
		...
		else
		{
			...
			pPool->m_pFreeBlock->freeUnit = pPool->m_pFreeBlock->freeUnit->nextUnit(pPool->m_nUnitSize);
		}
	...
	void freeFromPool(void* p, Pool* pPool)
	{
		...
		else
		{
			...
			if (pBlock->freeCount == 1)
			{
				...
				TZADDRESS nOffset = (TZADDRESS)((pPool->m_nMaxUnitCount - 1) * pPool->m_nUnitSize);
			...
			else
			{
				...
				else if (nAddress != (TZADDRESS)pBlock->freeUnit->prevUnit(pPool->m_nUnitSize))
				{
					((Unit*)nAddress)->next = pBlock->freeUnit;
	...
	void* reallocHeap(void* p, size_t newSize)
	{
		...
		Block * pBlock = (Block*)HeapCfg::sGetBlockAddress(p);
		TZ_ASSERT(pBlock);
		::memcpy(pNewMem, p, TZMIN(newSize, PoolCfg::sGetUnitSize(pBlock->poolIndex)));
	...
	PageHeap* getPageHeap(void *p)
	{
		...
		short nPageId = HeapCfg::sGetPageId(p);
	...
	void* reallocFromPagePool(PagePool* pPagePool)
	{
		...
		if (pPage == NULL)
		{
			...
			pPage->poolIndex = (short)pPagePool->m_nPoolIndex;
			pPage->memoryFlag &= ~HeapCfg::kAllocated;
			pPage->freeCount = (short)pPagePool->m_nMaxUnitCount;
		}
	...
	void* allocFromPagePool(PagePool* pPagePool)
	{
		...
		else
		{
			...
			pPagePool->m_pFreePageHeap->freeUnit = pPagePool->m_pFreePageHeap->freeUnit->nextUnit(pPagePool->m_nUnitSize);
		}
	...
	void freeFromPagePool(void *p, PagePool* pPagePool)
	{
		...
		else
		{
			...
			if (pPage->freeCount == 1)
			{
				...
				TZADDRESS nOffset = (TZADDRESS)((pPagePool->m_nMaxUnitCount - 1) * pPagePool->m_nUnitSize);
				if (nAddress == nBase + nOffset)
				{
					pPage->memoryFlag &= ~HeapCfg::kAllocated;
				}
			...
			else
			{
				...
				else if (nAddress != (TZADDRESS)pPage->freeUnit->prevUnit(pPagePool->m_nUnitSize))
				{
					((Unit*)nAddress)->next = pPage->freeUnit;
	...
	void* reallocHeapPage(void* p, size_t newSize)
	{
		...
		::memcpy(pNewMem, p, TZMIN(newSize, HeapCfg::sGetSizeOfPagePool(pPageHeap->poolIndex)));
	...
	void releasePagePool(int poolIndex)
	{
		...
		pPage = m_pagePools[poolIndex]->m_pFullPageHeap;
		while (pPage)
		{
			pPageToFree = pPage;
			pPage = pPage->next;
	...
	bool isFreeDelayEnabled() const
	{
		return m_bFreeDelay;
	}

	void enableFreeDelay()
	{
		m_bFreeDelay = true;
	}

	void disableFreeDelay()
	{
		m_bFreeDelay = false;
	}

	void setFreeDelay(bool flag)
	{
		m_bFreeDelay = flag;
	}
	...
	void initPageFlags()
	{
		HeapCfg::sInitPageFlags(m_pageFlags);
	}
...
private:
	...
	Page*					m_pFreePage;
	Page*					m_pFullPage;
	Page*					m_pagePools[HeapCfg::kMaxBlksPerPage + 1];
	PageHeap			m_pageHeaps[HeapCfg::kMaxPages];
	volatile short		m_nFreePageId;
...
#endif // #if (TZ_PLATFORM == TZ_PLATFORM_WINDOWS) && (TZ_ARCHITECURE_BIT == TZ_ARCHITECTURE_BIT64)
